        ...
        <div style={{ padding: 24, maxHeight: 420, overflowY: "auto" }}>
          {tab === "consultants" && (
            <div>
              <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
                ...
                <button onClick={addConsultant} style={{
                  padding: "8px 16px", borderRadius: 8, border: "none", cursor: "pointer",
                  background: `linear-gradient(135deg, ${CHOGAN_COLORS.gold}, ${CHOGAN_COLORS.goldLight})`,
                  color: CHOGAN_COLORS.darkGreen, fontFamily: "inherit", fontWeight: 700, whiteSpace: "nowrap"
                }}>+ Ajouter</button>
              </div>
              {consultants.map(c => (
          ...
          {tab === "questions" && (
            <div>
              ...
              <button onClick={addQuestion} style={{ padding: "10px 24px", borderRadius: 8, border: "none", cursor: "pointer", background: "rgba(255,255,255,0.08)", color: CHOGAN_COLORS.cream, fontFamily: "inherit", width: "100%", marginTop: 4 }}>+ Ajouter une question</button>
            </div>
          )}
        </div>
...
function GameScreen({ consultants: initConsultants, questions, onReset }) {
  ...
  const awardPoints = (consultantId, points) => {
    ...
    setAnswers(prev => ({ ...prev, [`${currentIdx}`]: { consultantId, correct: true, points } }));
  ...
  if (showFinal) return <FinalScreen consultants={[...consultants].sort((a,b) => b.score - a.score)} onReset={onReset} />;
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        @keyframes flash { 0%,100%{transform:scale(1)}50%{transform:scale(1.08)} }
        * { box-sizing: border-box; }
        input::placeholder { color: rgba(245,236,215,0.35); }
      `}</style>

      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 32px", borderBottom: `1px solid ${CHOGAN_COLORS.gold}22`, background: "rgba(0,0,0,0.2)" }}>
        <ChoganLogo />
        <h1 style={{ color: CHOGAN_COLORS.gold, fontFamily: "'Playfair Display', serif", fontSize: 32, margin: 0, letterSpacing: 8 }}>RÉBUS</h1>
        <div style={{ display: "flex", gap: 12 }}>
          ...
          <button onClick={() => setShowFinal(true)} style={{ background: "rgba(255,255,255,0.06)", border: `1px solid ${CHOGAN_COLORS.gold}33`, color: CHOGAN_COLORS.cream, borderRadius: 8, padding: "8px 16px", cursor: "pointer", fontFamily: "inherit", fontSize: 13 }}>🏆 Résultats</button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "300px 1fr 260px", overflow: "hidden" }}>
        <div style={{ padding: "24px 16px 24px 24px", borderRight: `1px solid ${CHOGAN_COLORS.gold}18`, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10 }}>
          {questions.map((q, i) => {
            ...
            const winnerC = done && answers[`${i}`] ? consultants.find(c => c.id === answers[`${i}`]?.consultantId) : null;
            return (
              <div key={q.id} onClick={() => { setCurrentIdx(i); setPhase(done ? "validate" : "question"); setImagePreview(null); }}
                style={{
                  padding: "12px 14px", borderRadius: 10, cursor: "pointer",
                  background: active ? `linear-gradient(135deg, ${CHOGAN_COLORS.gold}22, ${CHOGAN_COLORS.gold}08)` : "rgba(255,255,255,0.05)",
                  border: `1px solid ${active ? CHOGAN_COLORS.gold + "66" : "transparent"}`,
                  opacity: done && !active ? 0.65 : 1, transition: "all 0.2s"
                }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: "50%", flexShrink: 0, background: active ? `linear-gradient(135deg, ${CHOGAN_COLORS.gold}, ${CHOGAN_COLORS.goldLight})` : "rgba(255,255,255,0.08)", display: "flex", alignItems: "center", justifyContent: "center", color: active ? CHOGAN_COLORS.darkGreen : CHOGAN_COLORS.cream, fontWeight: 900, fontSize: 13 }}>{done ? "✓" : i + 1}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ color: active ? CHOGAN_COLORS.gold : CHOGAN_COLORS.cream, fontSize: 12, fontWeight: 700, lineHeight: 1.3, marginBottom: 3 }}>{done ? q.label : `Question ${i + 1}`}</div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ fontSize: 11, color: `${CHOGAN_COLORS.gold}aa` }}>{q.points} pts</span>
                      {winnerC && <span style={{ fontSize: 10, color: winnerC.color, fontWeight: 700 }}>✓ {winnerC.name}</span>}
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div style={{ padding: 32, display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ color: `${CHOGAN_COLORS.gold}99`, fontSize: 11, letterSpacing: 3, textTransform: "uppercase", marginBottom: 4 }}>Question {currentIdx + 1} / {questions.length}</div>
              <h2 style={{ color: CHOGAN_COLORS.cream, fontFamily: "'Playfair Display', serif", fontSize: 22, margin: 0 }}>{phase === "question" ? "🔍 Devinez le produit !" : currentQ.label}</h2>
            </div>
            <div style={{ background: `${CHOGAN_COLORS.gold}22`, border: `1px solid ${CHOGAN_COLORS.gold}55`, borderRadius: 10, padding: "10px 20px", textAlign: "center" }}>
          ...
          <div style={{ flex: 1, background: "rgba(255,255,255,0.03)", borderRadius: 16, border: `1px solid ${CHOGAN_COLORS.gold}18`, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", minHeight: 220, maxHeight: 300 }}>
            {imagePreview ? (
              <>
                ...
                {phase === "question" && (
                  <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ background: "rgba(13,56,38,0.6)", borderRadius: 12, padding: "12px 24px", border: `1px solid ${CHOGAN_COLORS.gold}33` }}>
                      <p style={{ color: CHOGAN_COLORS.gold, margin: 0, fontSize: 13, fontWeight: 700, letterSpacing: 2 }}>IMAGE MASQUÉE</p>
                    </div>
                  </div>
                )}
                <button onClick={() => fileRef.current.click()} style={{ position: "absolute", top: 10, right: 10, background: "rgba(0,0,0,0.4)", border: `1px solid ${CHOGAN_COLORS.gold}33`, color: CHOGAN_COLORS.cream, cursor: "pointer", borderRadius: 6, padding: "4px 10px", fontFamily: "inherit", fontSize: 11 }}>🔄</button>
              </>
            ) : (
              <div style={{ textAlign: "center", padding: 32 }}>
                <div style={{ fontSize: 48, marginBottom: 12, opacity: 0.4 }}>🧴</div>
                <button onClick={() => fileRef.current.click()} style={{ background: `${CHOGAN_COLORS.gold}18`, border: `1px dashed ${CHOGAN_COLORS.gold}44`, color: `${CHOGAN_COLORS.gold}aa`, cursor: "pointer", borderRadius: 8, padding: "10px 20px", fontFamily: "inherit", fontSize: 13 }}>📷 Ajouter une image produit</button>
              </div>
            )}
          ...
          {currentQ.hint && (
            <div style={{ padding: "10px 16px", borderRadius: 8, background: `${CHOGAN_COLORS.gold}0d`, border: `1px solid ${CHOGAN_COLORS.gold}22`, display: "flex", gap: 10, alignItems: "center" }}>
          ...
          {phase === "question" && (
            <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 12, padding: 20, border: `1px solid ${CHOGAN_COLORS.gold}22` }}>
              <p style={{ color: `${CHOGAN_COLORS.gold}cc`, fontSize: 12, letterSpacing: 2, textTransform: "uppercase", marginBottom: 14, marginTop: 0 }}>✏ Valider une réponse correcte</p>
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>
                {consultants.map(c => (
                  <button key={c.id} onClick={() => setSelectedConsultant(c.id === selectedConsultant ? null : c.id)}
                    style={{ padding: "8px 16px", borderRadius: 20, border: `2px solid ${c.id === selectedConsultant ? c.color : c.color + "44"}`, background: c.id === selectedConsultant ? c.color + "22" : "transparent", color: c.id === selectedConsultant ? c.color : `${c.color}99`, cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 700, transition: "all 0.15s" }}>{c.name}</button>
                ))}
              </div>
              <div style={{ display: "flex", gap: 10 }}>
                <input value={pendingAnswer} onChange={e => setPendingAnswer(e.target.value)} placeholder={`Réponse : « ${currentQ.answer} »`} style={inputStyle} onKeyDown={e => e.key === "Enter" && selectedConsultant && awardPoints(selectedConsultant, currentQ.points)} />
                <button onClick={() => selectedConsultant && awardPoints(selectedConsultant, currentQ.points)} disabled={!selectedConsultant}
                  style={{ padding: "10px 20px", borderRadius: 8, border: "none", cursor: selectedConsultant ? "pointer" : "not-allowed", background: selectedConsultant ? `linear-gradient(135deg, ${CHOGAN_COLORS.gold}, ${CHOGAN_COLORS.goldLight})` : "rgba(255,255,255,0.08)", color: selectedConsultant ? CHOGAN_COLORS.darkGreen : `${CHOGAN_COLORS.cream}44`, fontFamily: "inherit", fontWeight: 700, fontSize: 14, whiteSpace: "nowrap", transition: "all 0.2s" }}>✓ Valider</button>
                <button onClick={skipQuestion} style={{ padding: "10px 16px", borderRadius: 8, border: `1px solid rgba(255,80,80,0.3)`, background: "rgba(255,80,80,0.08)", color: "#e87a7a", cursor: "pointer", fontFamily: "inherit", fontSize: 13, whiteSpace: "nowrap" }}>Passer ✗</button>
              </div>
            </div>
          )}
          ...
          {phase === "validate" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12, alignItems: "center" }}>
              <div style={{ padding: "14px 28px", borderRadius: 10, background: `${CHOGAN_COLORS.gold}15`, border: `1px solid ${CHOGAN_COLORS.gold}44`, textAlign: "center" }}>
                {answers[`${currentIdx}`] ? (
                  <>
                    <span style={{ fontSize: 20 }}>🎉</span>
                    <span style={{ color: CHOGAN_COLORS.gold, fontWeight: 700, fontSize: 15, marginLeft: 8 }}>Réponse : <em>{currentQ.label}</em></span>
                    <span style={{ color: `${CHOGAN_COLORS.cream}99`, fontSize: 13, display: "block", marginTop: 4 }}>+{currentQ.points} pts → {consultants.find(c => c.id === answers[`${currentIdx}`]?.consultantId)?.name}</span>
                  </>
                ) : (
                  <span style={{ color: `${CHOGAN_COLORS.cream}88`, fontSize: 14 }}>❌ Passée — Réponse : <strong style={{ color: CHOGAN_COLORS.cream }}>{currentQ.answer}</strong></span>
                )}
              </div>
              <button onClick={nextQuestion} style={{ padding: "14px 40px", borderRadius: 10, border: "none", cursor: "pointer", background: `linear-gradient(135deg, ${CHOGAN_COLORS.gold}, ${CHOGAN_COLORS.goldLight})`, color: CHOGAN_COLORS.darkGreen, fontFamily: "inherit", fontWeight: 700, fontSize: 15, letterSpacing: 2, textTransform: "uppercase", boxShadow: `0 6px 20px ${CHOGAN_COLORS.gold}44` }}>
        ...
        <div style={{ padding: "24px 24px 24px 16px", borderLeft: `1px solid ${CHOGAN_COLORS.gold}18`, display: "flex", flexDirection: "column", gap: 12 }}>
          <p style={{ color: `${CHOGAN_COLORS.gold}99`, fontSize: 11, letterSpacing: 3, textTransform: "uppercase", margin: "0 0 8px" }}>SCORES CONSULTANTES</p>
          {[...consultants].sort((a, b) => b.score - a.score).map((c, i) => (
            <div key={c.id} style={{ padding: "14px 16px", borderRadius: 12, background: flashId === c.id ? `${c.color}22` : "rgba(255,255,255,0.04)", border: `1px solid ${i === 0 && c.score > 0 ? c.color + "44" : CHOGAN_COLORS.gold + "15"}`, animation: flashId === c.id ? "flash 0.8s ease" : "none", transition: "background 0.3s" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ fontSize: 16 }}>{i === 0 && c.score > 0 ? "👑" : i === 1 ? "🥈" : "🥉"}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ color: c.color, fontWeight: 700, fontSize: 14 }}>{c.name}</div>
                  <div style={{ display: "flex", gap: 4, marginTop: 2 }}>{Array.from({ length: Math.min(5, Math.floor(c.score / 10)) }).map((_, j) => <Star key={j} />)}</div>
                </div>
              ...
              <div style={{ marginTop: 10, height: 3, background: "rgba(255,255,255,0.08)", borderRadius: 2 }}>
                <div style={{ height: "100%", borderRadius: 2, width: `${Math.max(4, (c.score / Math.max(1, Math.max(...consultants.map(x => x.score)))) * 100)}%`, background: c.color, transition: "width 0.5s ease" }} />
              </div>
            </div>
          ))}
          <div style={{ marginTop: "auto", padding: "14px 16px", borderRadius: 12, background: "rgba(255,255,255,0.03)", border: `1px solid ${CHOGAN_COLORS.gold}15` }}>
            ...
            <div style={{ height: 4, background: "rgba(255,255,255,0.08)", borderRadius: 4 }}>
              <div style={{ height: "100%", borderRadius: 4, width: `${(revealed.length / questions.length) * 100}%`, background: `linear-gradient(90deg, ${CHOGAN_COLORS.gold}, ${CHOGAN_COLORS.goldLight})`, transition: "width 0.4s ease" }} />
            </div>
            <p style={{ color: `${CHOGAN_COLORS.cream}66`, fontSize: 12, margin: "8px 0 0", textAlign: "right" }}>{revealed.length} / {questions.length} questions</p>
          </div>
        </div>
      </div>
    </div>
...
function FinalScreen({ consultants, onReset }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: `radial-gradient(ellipse at center, #1a5c3a 0%, ${CHOGAN_COLORS.darkGreen} 60%, #050f08 100%)`, fontFamily: "'Lato', sans-serif", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32 }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700;900&family=Lato:wght@300;400;700&display=swap');
      ...
      <div style={{ textAlign: "center", marginBottom: 48, animation: "fadeUp 0.8s ease" }}>
        ...
        <h1 style={{ color: CHOGAN_COLORS.gold, fontFamily: "'Playfair Display', serif", fontSize: 48, margin: "0 0 8px", textShadow: `0 0 40px ${CHOGAN_COLORS.gold}88` }}>FIN DU JEU</h1>
        <p style={{ color: `${CHOGAN_COLORS.cream}88`, fontSize: 16, letterSpacing: 4, textTransform: "uppercase" }}>Rébus Chogan — Résultats finaux</p>
      </div>
      {winner && winner.score > 0 && (
        <div style={{ textAlign: "center", marginBottom: 40, animation: "fadeUp 0.8s ease 0.2s both" }}>
          <div style={{ display: "inline-block", padding: "24px 48px", borderRadius: 20, background: `${CHOGAN_COLORS.gold}18`, border: `2px solid ${CHOGAN_COLORS.gold}55`, boxShadow: `0 0 60px ${CHOGAN_COLORS.gold}22` }}>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", justifyContent: "center", marginBottom: 48 }}>
        {consultants.map((c, i) => (
          <div key={c.id} style={{ padding: "20px 28px", borderRadius: 16, background: i === 0 ? `${c.color}18` : "rgba(255,255,255,0.04)", border: `1px solid ${i === 0 ? c.color + "55" : CHOGAN_COLORS.gold + "18"}`, textAlign: "center", minWidth: 140, animation: `fadeUp 0.6s ease ${0.4 + i * 0.1}s both` }}>
      ...
      <button onClick={onReset} style={{ padding: "16px 48px", borderRadius: 12, border: "none", cursor: "pointer", background: `linear-gradient(135deg, ${CHOGAN_COLORS.gold}, ${CHOGAN_COLORS.goldLight})`, color: CHOGAN_COLORS.darkGreen, fontFamily: "inherit", fontSize: 16, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", boxShadow: `0 8px 24px ${CHOGAN_COLORS.gold}44` }}>
        🔄 NOUVELLE PARTIE
      </button>
    </div>
  );
